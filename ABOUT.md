...
The idea crystallized around two converging trends: the rise of agentic AI (LLMs that can reason, plan, and use tools in a loop) and the availability of deliberately vulnerable applications like OWASP Juice Shop that provide realistic attack surfaces. We were inspired by the ShannonAI paradigm (information-theoretic reconnaissance — maximize signal, minimize noise) and the PentAI concept (autonomous agents with structured exploit chains). PenteraX merges both: a hybrid system where deterministic pre-collection feeds high-quality context to an LLM agent that reasons about vulnerabilities, generates hypotheses, crafts exploits, and writes a professional report — all without human intervention.
